...
class Update extends Component {
   constructor(props) {
    ...
    this.state = {
     ...
     if (props.params.id !== undefined) {
      props.fetchOneUser("", props.params.id)
    }
  }
  componentWillReceiveProps(props) {
    ...
    if (props.user !== undefined && props.user.action === types.user.USER_UPDATE) {
       OurToaster.show({ intent: Intent.SUCCESS, message: "Cập nhật thành công!" });
       browserHistory.push('/user/update/'+this.state.id)
    }
    if (props.user !== undefined && props.user.action === types.user.USER_UPDATE_CLEAR) {
       OurToaster.show({ intent: Intent.DANGER, message: "Cập nhật thất bại!" });
    }
  ...
  onSubmit(e) {
    ...
    this.props.updateUser(this.state.id,params)
              ...
              <div className="panel panel-default">
                <div className="panel-heading clearfix">
                  <div className="col-md-5">
                    <h4 className="panel-title"> <a data-toggle="collapse" href="#plg" > Cập nhật thông tin tài khoản</a> </h4>
                  </div>
                </div>
                        ...
                        <div className="panel panel-default">
                          ...
                          <div className="panel-body">
                              ...
                              <div  className="form-group">
                                ...
                                <input disabled type="userName" value={this.state.userName} name="userName" className="form-control"
                                  onChange={(e) => this.setState({ userName: e.target.value })}
                                  />
                              </div>
                              <div  className="form-group">
                                <label>Email </label>
                                <input disabled type="email" value={this.state.email} name="email" className="form-control"
                                  onChange={(e) => this.setState({ email: e.target.value })}
                                  />
                              </div>
                              <div  className="form-group">
                                <label>Trạng thái </label>
                                <select disabled name="active" value={this.state.active} className="form-control"
...
export default connect(mapStateProps, mapDispatchToProps)(Update)